...
| T6.13 | PWA install banner, phone-only: `manifest.ts` (standalone, token colors, 192/512 + maskable icons), `src/lib/device.ts` (isAndroidPhone/isIPhone/isStandalone — iPad excluded incl. the iPadOS-13+ Macintosh-UA masquerade via maxTouchPoints), `install-banner.tsx` in the (app) layout: Android captures `beforeinstallprompt` → native install dialog; iPhone shows Share → Add to Home Screen (no fake button); X = session-only dismiss (plain state, no storage); never on desktop/iPad/standalone | `tests/device.test.ts` UA table (Android phone/tablet, iPhone, legacy iPad, iPad-as-Mac, macOS, Windows, Firefox Android); design-qa auto-covers the tsx; live E2E: manifest served, iPhone banner + dismiss + refresh-returns, synthetic beforeinstallprompt on Android context, absent on desktop/iPad | ✅ |
| T6.12 | Performance audit (`docs/performance-audit.md`): 14-item checklist triaged with code evidence — 11 rejected (platform/framework/already shipped), 3 done: lazy blob images (`loading="lazy"` on all four `<img>`s), removed never-imported deps `swr` + `date-fns` (Tech guide's wrong SWR claim corrected, PDF regenerated), Lighthouse baseline measured 98/100/100/100 | `pnpm verify` + build green after dep removal; regenerated tech-guide HTML greps 0×SWR/0×date-fns; Lighthouse JSON in `.uat-build/` | ✅ |
| T6.11 | Loading skeletons (`docs/loading-skeletons.md`): route-group `(app)/loading.tsx` paints instantly on every navigation while the server renders (fixes the frozen-click gap, 500–1100ms measured); dashboard gets a shape-parity skeleton (stat grid + chart shells); primitives in `skeleton.tsx` — clay surfaces, `bg-line` token, opacity-only pulse, `role="status"` a11y | design-qa statically covers the new tsx (text sizes/hex/blur); build compiles the loading boundaries; live Playwright check with a delayed navigation shows the skeleton then the real page | ✅ |
| T6.10 | Response caching (D-030): in-process TTL cache (`src/lib/ttl-cache.ts`) behind dashboard/monitoring aggregates (45s), quick-search (60s + private browser cache), Sphera guide config (60s) and empty-thread Sphera FAQ replies (10min, turn still persisted); explicit `no-store` on audited export/FHIR responses and badges | `tests/ttl-cache.test.ts` (expiry, dedupe, eviction, retry-after-failure); guide-chat cache tests: identical empty-thread question skips the model but persists, history/greetings never cached | ✅ |
| T6.9 | Exports page shows per-trial row counts (`exportRowCounts` in sdtm.ts): participant/AE badges on each trial card, per-format row counts on the download buttons, and an explicit headers-only note when a trial has 0 AEs — an empty-but-valid SDTM AE CSV is expected before download, not mistaken for a broken export (live DB had zero AEs after the real-CTRI reseed) | PGlite: counts match domain row counts for AYU-001 (45/3) and a 0-AE trial's AE CSV is exactly the header line | ✅ |
| T6.8 | Data-query answer flow reachable from Monitoring: query rows link to their visit page (where the `crf.enter` roles' **Answer** form lives) and open queries show an explicit "Answer on the visit page →" CTA; `listQueries` returns `visitId`; monitor guide workflow updated | Existing data-queries + guide drift-guard suites; live E2E: raise → answer via link → answered badge → close | ✅ |
| T6.7 | Timezone-correct date rendering (bug: monitoring visits completed after midnight IST showed the previous day — Vercel renders server components in UTC): new `src/lib/dates.ts` (`fmtDate`/`fmtDateTime`/`fmtTime`, en-IN + Asia/Kolkata) replaces all 40 `toLocale*String` call sites app-wide; monitoring visit list now orders newest-created first with explicit "scheduled/completed" labels (prevents completing a neighbouring card) | `tests/dates.test.ts` pins the exact live-repro instant (13/09 19:37 UTC → "14/09/2026"); design-qa static guard forbids bare `toLocale*String` without APP_LOCALE; live DB audit rows verified stored instants were always correct | ✅ |
| T6.6 (kept current through T6.13: IST dates, D-030 caching, skeletons, Lighthouse 98/100/100/100, PWA, 319 tests) | Judge-facing Technology & Security Guide (`scripts/build-tech-guide.ts` → `docs/uat/Tech_Security_Guide.pdf` + `.md`): full stack table (versions read from package.json), request/Sphera/interop workflow diagrams, privacy-vs-security explainer, every implemented security layer (auth, RBAC matrix rendered from `rbac.ts`, withAudit, e-signatures, Zod/Drizzle, secrets, cron), DPDP privacy section, differentiators, honest hardening roadmap | Typecheck covers the generator; RBAC matrix grant count in output equals the code matrix (57); key-string sweep of the generated HTML; visual check of matrix + flow renders | ✅ |
| T6.5 | UAT guides regenerated for the post-gap-fill app (`scripts/build-uat-guides.ts`): all 7 role PDFs rebuilt from the living knowledge base (`knowledge.ts` workflows + `groupedNavForRole` sidebar maps) with fresh screenshots of the deployed site, per-role "New in this build" and evaluation-axis mapping; new `Judge_Demo_Script.pdf` + `.md` (8-minute cross-role storyline mapped to the 4 axes). Knowledge base updated for randomization, signed approvals, amendments, consent binding, data queries, monitoring visits, alert-config, ADSL/Define-XML/FHIR API | Guide + drift-guard suites green after knowledge updates; generator typechecks; PDFs re-runnable via `pnpm tsx scripts/build-uat-guides.ts`; key screenshots/HTML reviewed | ✅ |
| T6.4 | UI/UX audit + fixes ([ui-audit.md](ui-audit.md)): grouped sidebar (6 sections for roles >6 items, compact rows, slim footer — `groupedNavForRole` in nav.ts, shared `NavList`), mobile hamburger drawer (phones previously had NO navigation), quick-jump search replacing the dead topbar box (`quickSearch` service + `/api/search`, RBAC-shaped), Adverse Events page split into tabs (Open/Capture/Signals/ADRs); guide knowledge updated | `tests/nav.test.ts` grouping specs (RBAC-exact, header threshold >6, admin all sections, regulator never Settings) + `tests/search.test.ts` (trials/participants/screens buckets per role, caps, short-query guard); build green; 7-role Playwright sweep on the deployed site | ✅ |
| T6.3 | Sphera v2 (D-029): LangChain + LangGraph rebuild with Neon-persisted memory — `guide_messages` threads per user, `StateGraph` + ChatGroq (20s timeout, low reasoning effort for gpt-oss), server-side history (client sends only message+threadId), GET rehydration + "New chat", 3s cap on the config read + `maxDuration` 60 (504 hardening). Admin-stored Settings key keeps powering all roles | `tests/guide-chat.test.ts`: turn streams + persists both sides; second turn's model input provably contains the first (memory); greeting stores only the reply; threads isolated per user/threadId; history trimmed to MAX_HISTORY_TURNS; empty turn refused. Full suite + build green | ✅ |
